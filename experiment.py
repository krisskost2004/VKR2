...
class ComparativeExperiment:
    ...
    def __init__(self, num_runs: int = 10, max_iter: int = 100, pop_size: int = 30,
    ...
    def evaluate_solution(self, problem_name: str, solution: np.ndarray) -> Dict[str, Any]:
        ...
        if problem_name == 'dc_motor_pid':
            ...
            return {'feasible': feasible, 'acceptable': acceptable,
            ...
            try:
                ...
                feasible = (not np.any(np.isnan(h1_traj)) and not np.any(np.isnan(h2_traj)) and
                            not np.any(np.isinf(h1_traj)) and not np.any(np.isinf(h2_traj)) and
                            np.all(np.array(h1_traj) >= -0.01) and np.all(np.array(h2_traj) >= -0.01))
                if not feasible:
                    return {'feasible': False, 'acceptable': False, 'metrics': {}}

                last_t = times[-1]
                start_eval = last_t * 0.8
        ...
        try:
            ...
            if algorithm_name == 'PSO':
                algorithm = algorithm_class(
                    objective_func=objective_func, dim=dim, bounds=bounds,
                    max_iter=self.max_iter, pop_size=self.pop_size,
                ...
                algorithm = algorithm_class(
                    objective_func=objective_func, dim=dim, bounds=bounds,
                    max_iter=self.max_iter, pop_size=self.pop_size, seed=seed
                )
            ...
            # Вычисляем J@500, J@1000, J@2000 с интерполяцией
            J_at_500 = None
            J_at_1000 = None
            J_at_2000 = None
            if 'convergence_history' in metrics and metrics['convergence_history']:
                hist = metrics['convergence_history']  # список лучших фитнесов после каждой итерации
